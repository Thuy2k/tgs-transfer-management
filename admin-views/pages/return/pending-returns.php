<?php
/**
 * Danh sách phiếu chờ nhận hàng trả
 *
 * Hiển thị các return đang pending cần nhận hàng (shop mẹ nhận từ shop con)
 *
...
<div class="app-pending-returns">
    ...
    <div id="emptyState" class="card d-none">
        <div class="card-body text-center py-5">
            ...
            <p class="text-muted mb-0">Hiện tại không có hàng trả nào đang chờ nhận từ shop con.</p>
        </div>
    </div>
...
jQuery(document).ready(function($) {
    ...
    function loadPendingReturns() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'tgs_return_get_pending_returns',
                nonce: nonce
            },
    ...
    function renderPendingReturns(returns) {
        ...
        returns.forEach(function(item) {
            const statusClass = item.return_status == 1 ? 'approved' : '';
            const statusBadge = item.return_status == 1
            ...
            html += `
                <div class="card mb-3 pending-return-card ${statusClass}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="collapse" id="products-${item.return_id}">
                            <div class="border-top pt-3">
                                <h6 class="mb-2">Danh sách sản phẩm:</h6>
                                <div class="products-container" id="products-list-${item.return_id}">
    ...
    // Load products for a return
    function loadReturnProducts(returnId) {
        $.ajax({
            url: ajaxUrl,
            type: 'POST',
            data: {
                action: 'tgs_return_get_items',
                nonce: nonce,
                return_id: returnId
            },
            success: function(response) {
                if (response.success && response.data) {
                    renderReturnProducts(returnId, response.data);
                } else {
                    $(`#products-list-${returnId}`).html('<div class="text-muted">Không có sản phẩm</div>');
                }
            },
            error: function() {
                $(`#products-list-${returnId}`).html('<div class="text-danger">Lỗi tải sản phẩm</div>');
            }
        });
    }

    // Render products for a return
    function renderReturnProducts(returnId, items) {
        let html = '';
        ...
        $(`#products-list-${returnId}`).html(html || '<div class="text-muted">Không có sản phẩm</div>');
    }
    ...
    $(document).on('click', '.btn-toggle-products', function() {
        ...
        const target = $(`#products-${returnId}`);

        if (!loaded || loaded === 'false') {
            loadReturnProducts(returnId);
